...
class AudacityConnector(BaseConnector):
    """
    Orchestrates metadata extraction for Audacity project files (.aup3).
    """
    ...
    def _get_k8s_job_definition(self, asset_id: str, file_uri: str) -> client.V1Job:
        """Creates the Kubernetes Job object for the Audacity processor."""
        job_name = f"audacity-processor-{asset_id[:8]}-{uuid.uuid4().hex[:6]}"
        processor_image = "platformq/audacity-processor:latest"
        file_path_in_pod = f"/data/{file_uri.split('/')[-1]}"

        container = client.V1Container(
            name="audacity-processor",
        ...
        pod_template = client.V1PodTemplateSpec(
            metadata=client.V1ObjectMeta(name=job_name, labels={"app": "audacity-processor"}),
