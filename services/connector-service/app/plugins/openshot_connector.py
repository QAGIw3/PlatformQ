...
class OpenshotConnector(BaseConnector):
    """
    Orchestrates metadata extraction for OpenShot project files (.osp).
    """
    ...
    def _get_k8s_job_definition(self, asset_id: str, file_uri: str) -> client.V1Job:
        """Creates the Kubernetes Job object for the OpenShot processor."""
        job_name = f"openshot-processor-{asset_id[:8]}-{uuid.uuid4().hex[:6]}"
        processor_image = "platformq/openshot-processor:latest"
        file_path_in_pod = f"/data/{file_uri.split('/')[-1]}"

        container = client.V1Container(
            name="openshot-processor",
        ...
        pod_template = client.V1PodTemplateSpec(
            metadata=client.V1ObjectMeta(name=job_name, labels={"app": "openshot-processor"}),
