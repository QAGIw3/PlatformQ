...
async def get_lineage_tracker(request: Request) -> MLModelLineageTracker:
    """Get ML lineage tracker instance"""
    tracker = request.app.state.ml_lineage_tracker
    if tracker is None:
        raise HTTPException(
            status_code=503,
            detail="ML lineage tracking is not available. JanusGraph may not be configured."
        )
    return tracker
