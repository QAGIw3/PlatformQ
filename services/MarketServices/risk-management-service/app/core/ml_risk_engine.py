...
class MLRiskEngine:
    ...
    async def assess_position_risk(
        ...
        # Margin utilization
        margin_utilization = Decimal(str(position.get("margin_used", 0))) / Decimal(str(position.get("collateral_value", 1)))
    ...
    async def _calculate_var(
        ...
        volatility_adjustment = float(market_data.get("volatility_24h", 1)) / float(market_data.get("avg_volatility", 1))
    ...
    async def _initialize_models(self):
        ...
        try:
            ...
            logger.info("Pre-trained models not found, initializing default models")
