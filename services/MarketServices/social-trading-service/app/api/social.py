...
from app.dependencies import (
    get_settings, get_copy_executor, get_reputation_engine,
    get_current_user, get_cache_manager
)
...
async def start_copy_trading(
    ...
    user_id: Annotated[str, Depends(get_current_user)],
    copy_executor: Annotated[CopyTradingExecutor, Depends(get_copy_executor)],
    cache_manager = Depends(get_cache_manager),
    ...
    # Check copy allocation limit
    if request.allocation_percent and request.allocation_percent > settings.max_copy_allocation * 100:
        raise HTTPException(
            status_code=400,
            detail=f"Maximum allocation is {settings.max_copy_allocation * 100}%"
        )
...
async def get_active_copy_relations(
    ...
    return await cache_manager.get_user_copy_relations(user_id)
...
async def create_post(
    ...
    if daily_posts >= settings.max_posts_per_day:
...
async def get_leaderboard(
    ...
    reputation_engine: Annotated[ReputationEngine, Depends(get_reputation_engine)],
    cache_manager = Depends(get_cache_manager),
    settings: Annotated[Settings, Depends(get_settings)]
):
    ...
    if not leaderboard:
        ...
        top_traders = await reputation_engine.get_top_traders(
            limit=limit,
            min_trades=settings.min_trades_for_reputation
        )
