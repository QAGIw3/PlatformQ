...
from ..openproject_client import openproject_client

class CRUDProject:
    def create(self, *, obj_in: ProjectCreate, owner_id: str) -> Project:
        # In a real scenario, you would map the owner_id to an OpenProject user
        project_data = {"title": obj_in.title, "description": obj_in.description}
        created_project = openproject_client.create_project(project_data)
        
        milestones = []
        for ms_in in obj_in.milestones:
            ms_data = {"title": ms_in.title, "description": ms_in.description}
            created_ms = openproject_client.create_work_package(created_project["id"], ms_data)
    ...
    def get(self, id: UUID) -> Optional[Project]:
        project_data = openproject_client.get_project(str(id))
        if project_data:
            # This is simplified; in a real app, you'd fetch work packages (milestones)
            # associated with the project.
            return Project(
                id=UUID(project_data["id"]),
                owner_id="mock_owner", # Placeholder
    ...
    def get_multi(self) -> List[Project]:
        projects_data = openproject_client.get_projects()
        return [
            Project(
                id=UUID(p["id"]),
                owner_id="mock_owner",
    ...
    def complete_milestone(self, project_id: UUID, milestone_id: UUID, user_id: str) -> Optional[Milestone]:
        updated_package = openproject_client.update_work_package_status(str(milestone_id), "Closed")
        if updated_package:
            event_data = {
