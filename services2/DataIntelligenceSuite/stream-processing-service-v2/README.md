...
|----------|-------------|---------|
| SERVICE_NAME | Service identifier | stream-processing-service-v2 |
...
```bash
# Build image
docker build -t stream-processing-service-v2:latest .

# Run container
docker run -p 8000:8000 stream-processing-service-v2:latest
...
# Or using Helm
helm install stream-processing-service-v2 ./helm
```
